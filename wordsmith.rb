...
def zip()
  ...
  puts "Zip codes in #{@stateAbbrev}:     #{zipspp.length}"
end
...
def areaCode()
  ...
  areapp = postProcessor(stateLine)
  @areaCode = @areaCode + areapp

  if @quiet != true
    puts areapp
    puts ""
  end
  puts "Area codes in #{@stateAbbrev}:    #{areapp.length}"
end
...
def names()
  ...
  allNamespp = postProcessor(allNames)

  @allNames = @allNames + allNamespp
  if @quiet != true
    puts allNamespp
    puts ""
  end
  puts "Common names in USA: #{allNamespp.length}"
end
